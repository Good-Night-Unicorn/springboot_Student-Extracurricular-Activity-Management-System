<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="活动人数" prop="huodongrenshu">
				<el-input v-model.number="ruleForm.huodongrenshu" 
					placeholder="活动人数" clearable :disabled=" false  ||ro.huodongrenshu"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="时间" prop="shijian">
				<el-date-picker
					:disabled=" false  ||ro.shijian"
			...
			<el-form-item class="add-item" label="活动描述" prop="huodongmiaoshu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="活动描述"
			...
			<el-form-item class="add-item" label="报名要求" prop="baomingyaoqiu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="报名要求"
			...
			<el-form-item class="add-item" label="活动详情" prop="huodongxiangqing">
				<editor 
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`huodongxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

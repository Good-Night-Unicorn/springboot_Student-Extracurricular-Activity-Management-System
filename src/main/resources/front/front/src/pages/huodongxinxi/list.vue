	...
	<div>
		...
		<div class="list-preview">
			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('huodongxinxi','新增')" type="primary" @click="add('/index/huodongxinxiAdd')">
				...
				<div class="list3 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="item-info">
							...
							<div class="desc ql-snow ql-editor" v-html="item.huodongmiaoshu"></div>
						</div>
					</div>
				</div>
			</div>
...
<script>
	export default {
		...
		methods: {
			...
			async getFenlei() {
				await this.$http.get('huodongfenlei/list',{params: {sort: 'huodongfenlei',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
						this.fenlei = res.data.data.list
					}
				});
				this.feileiColumn = 'huodongfenlei'
			},
			getList(page, fenlei, ref = '') {
				...
				if (this.curFenlei != '全部') searchWhere.huodongfenlei = this.curFenlei;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`huodongxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/huodongxinxiDetail', query: params});
			},

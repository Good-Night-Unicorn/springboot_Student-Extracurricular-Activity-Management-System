<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="星级评价" prop="xingjipingjia">
				<el-select v-model="ruleForm.xingjipingjia" placeholder="请选择星级评价" :disabled=" false  ||ro.xingjipingjia" >
					<el-option
			...
			<el-form-item class="add-item" label="评价活动" prop="pingjiahuodong">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="评价活动"
					...
					if(valid) {
						if(this.type=='cross'){
							...
							if(crossrefid && crossuserid) {
								...
								await this.$http.get('huodongpingjia/list', {
									params: params
								}).then(res => {
						...
						await this.$http.post(`huodongpingjia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',

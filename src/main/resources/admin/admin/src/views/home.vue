<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('huodongxinxi','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{huodongxinxiCount}}</div>
					<div class="name">活动信息总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('huodongbaoming','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{huodongbaomingCount}}</div>
					<div class="name">活动报名总数</div>
				</div>
			</div>
			<div id="statis3" class="statis3 animate__animated" v-if="isAuth('huodongpingjia','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-qita6"></span>
				</div>
				<div class="right">
					<div class="num">{{huodongpingjiaCount}}</div>
					<div class="name">活动评价总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 4 -->
		<div class="type4">
			<div id="huodongxinxiChart1" class="echarts1 animate__animated" v-if="isAuth('huodongxinxi','首页统计')"></div>
			<div id="huodongbaomingChart1" class="echarts2 animate__animated" v-if="isAuth('huodongbaoming','首页统计')"></div>
			<div id="huodongbaomingChart2" class="echarts3 animate__animated" v-if="isAuth('huodongbaoming','首页统计')"></div>
			<div id="huodongpingjiaChart1" class="echarts4 animate__animated" v-if="isAuth('huodongpingjia','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		if(this.isAuth('huodongxinxi','首页统计')){
			this.huodongxinxiChat1();
		}
		this.gethuodongbaomingCount();
		if(this.isAuth('huodongbaoming','首页统计')){
		...
		if(this.isAuth('huodongpingjia','首页统计')){
			this.huodongpingjiaChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'huodongxinxiChart1',css:'animate__fadeInUp'},
				{id:'huodongbaomingChart1',css:'animate__fadeInUp'},
				{id:'huodongbaomingChart2',css:'animate__fadeInUp'},
				{id:'huodongpingjiaChart1',css:'animate__fadeInUp'},
			]
...
// 1234 饼
		huodongpingjiaChat1() {
			this.$nextTick(()=>{
				...
				var huodongpingjiaChart1 = echarts.init(document.getElementById("huodongpingjiaChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						huodongpingjiaChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							huodongpingjiaChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
...
<style lang="scss" scoped>
	.home-content {
		...
		margin: 0 auto;
		background: url(http://codegen.caihongy.cn/20250330/347599fc0a64428593e6e346c27c3009.png) no-repeat center top / cover;
		display: flex;
		width: 100%;
		min-height: 100vh;
		...
		.statis-box {
			...
			.statis1 {
				...
				.left {
					border-radius: 16px;
					background: url(http://codegen.caihongy.cn/20241124/818f3019b58b4718b20bd79e06f244ad.png) no-repeat center center;
					display: flex;
					width: 56px;
					justify-content: center;
					align-items: center;
